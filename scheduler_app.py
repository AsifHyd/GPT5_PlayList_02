    ...
    def set_start_for_selected(self):
        ...
        for i in sel:
            if 0 <= i < len(self.videos):
                self.videos[i]['absolute_time'] = s
        self.update_timeline()
        self.status_var.set(f"Exact start {t} set for {len(sel)} item(s)")
        ...
        try:
            if self.obs_client:
                ...
                if input_name:
                    ...
                    ps = cur / 1000 if dur >= 1000 else cur
                    ts = dur / 1000 if dur >= 1000 else dur
                    rem = max(ts - ps, 0)
